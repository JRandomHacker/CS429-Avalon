...
void GameWindow::createPlayerSubscribers( ) {
    ...
    ui->stateLabel->setText( "Team Select" );
    ...
    for(unsigned int i = 0; i < qLength; i++) {
        ...
        voteTrackLabels[i]->setToolTip( "No quest history to display." );
...
void GameWindow::updatePlayer( unsigned int id ) {
    ...
    if( p != NULL ) {
        ...
        if( id == *myID_subscriber->getData<unsigned int>( ) ) {
            ...
            ui->playerAvatarLabel->setToolTip( avalon::gui::roleToFlavorText( p->getRole(), p->getAlignment() ).c_str( ) );
        ...
        listModel->setItem( id, 1, new QStandardItem(
                                avalon::gui::alignmentToString( p->getAlignment( ) ).c_str( ) ) );

        listModel->setItem( id, 2, new QStandardItem(
                                avalon::gui::roleToString( p->getRole( ) ).c_str( ) ) );
    }
    listModel->setItem( id, 0, new QStandardItem( newString.c_str( ) ) );
...
void GameWindow::updateLeader( ) {
    ...
    if( lid == myID ) {
        ...
        ui->leaderLabel->setText( "You are the leader!" );
        ...
        if( leader != NULL ) {
            ui->leaderLabel->setText( ( "Current Leader: "+leader->getName( ) ).c_str( ) );
        }
        ui->proposeTeamButton->hide( );
    } else {

        ui->leaderLabel->setText( "" );
        ui->proposeTeamButton->hide( );
    }
}
...
void GameWindow::updateQuestingTeam( ) {
    ...
    for( unsigned int i = 0; i < team.size( ); i++ ) {
        ...
        qModel->appendRow( new QStandardItem( p->getName( ).c_str( ) ) );

        QStringList qHeaders;
        qHeaders.append( "Name" );
    ...
    ui->teamSizeLabel->setText( (std::to_string( team.size( ) ) + "/" + std::to_string( playersCurrQuest )
...
void GameWindow::updateGameInfo( ) {
    ...
    ui->numOfEvilPlayers->setText( num_evil_string.c_str( ) );
...
void GameWindow::updateTrack( ) {
    ...
    if(currQuest > lastUpdatedQuest) {
        ...
        voteTrackLabels[currQuest-1]->setToolTip( buildQuestHistoryString( ).c_str( ) );
...
void GameWindow::updateTeamVoteState( ) {
    ...
    if( inVoteState ) {
        ...
        ui->stateLabel->setText( "Team Vote" );
        ...
        listModel->setHorizontalHeaderItem( 3, new QStandardItem( "Has Voted" ) );
...
void GameWindow::updateQuestVoteState( ) {
    ...
    if( inQuestVote ) {
        ui->stateLabel->setText( "Quest Vote" );

        // Create the "Has Voted" column
        for( int i = 0; i < listModel->rowCount( ); i++ ) {
            listModel->setItem( i, 1, new QStandardItem( "No" ) );
        }
        listModel->setHorizontalHeaderItem( 1, new QStandardItem( "Has Voted" ) );
        ...
        ui->stateLabel->setText( "Team Selection" );
        ui->votingSection->hide( );
...
void GameWindow::updateAssassinState( ) {
    ...
    if( inAssassinState ) {
        ui->stateLabel->setText( "Assassin Round" );
        ui->teamSizeLabel->hide( );
        ui->votingSection->hide( );
        ui->proposedTeamSection->setTitle( "Assassination" );

        assassinChoice = -1;

        if( amAssassin ) {
            ui->leaderLabel->setText( "You are the assassin!  Choose a player to kill." );
            ui->proposeTeamButton->setText( "Assassinate" );
            ui->proposeTeamButton->show( );
            ui->proposeTeamButton->setEnabled( false );
            ...
            ui->leaderLabel->setText( "An assassin walks among you..." );
            ui->proposeTeamList->hide( );
            ui->proposeTeamButton->hide( );
        }
    }
...
void GameWindow::updateEndGameState( ) {
    ...
    if( inEndGameState ) {
        ...
        ui->stateLabel->setText( "Game Over" );
    ...
    if( *teamVoteState_subscriber->getData<bool>( ) ) {
        for( unsigned int i = 0; i < votes.size( ); i++ ) {
            if( votes[i] != avalon::NO_VOTE ) {
                QStandardItemModel* listModel = ( QStandardItemModel* ) ui->playerList->model( );
                listModel->item( i, 3 )->setText( "Yes" );
            }
        }
    } else {
        std::vector<unsigned int> team = *questingTeam_subscriber->getData<std::vector<unsigned int>>( );
        ...
        for( unsigned int i = 0; i < votes.size( ); i++ ) {
            if( votes[i] != avalon::NO_VOTE ) {
                ...
                QStandardItemModel* listModel = ( QStandardItemModel* ) ui->proposeTeamList->model( );
                listModel->item( listPos, 1 )->setText( "Yes" );
            }
        }
    }
}
...
void GameWindow::updateChatMessages( ) {
    ...
    QStandardItem* messageItem = new QStandardItem( messageStr.c_str( ) );
...
void GameWindow::on_playerList_clicked( const QModelIndex& index ) {
    ...
    if( *assassinState_subscriber->getData<bool>( ) && amAssassin ) {
        ...
        QStandardItem* choiceItem = new QStandardItem( chosenName.c_str( ) );
